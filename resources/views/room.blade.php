                ...
                <div class="table-toolbar">
                    <div class="btn btn-group" style="margin-bottom: 20px;">
                        <a id="editable_table_new" class="add btn btn-primary glow_button hvr-float-shadow" style="color:white; font-size:17px">
                            Add Room &nbsp;&nbsp;&nbsp; <i class="fa fa-plus-circle"></i>
                        </a>
                    </div>
                                ...
                                <tr role="row" class="even">
                                    ...
                                    <td>
                                        <div class="btn-group button_group_rounded btn_group_padding" role="group">
                                            <button type="button" class="btn btn-outline-secondary edit" data-toggle="tooltip" data-id="{{ $room->id }}" data-placement="top" title="Edit"><i style="font-size:x-large" class="fa fa-pencil text-warning"></i></button>
                                            <button type="button" class="btn btn-outline-secondary delete" data-toggle="tooltip" data-id="{{ $room->id }}" data-placement="top" title="Remove"><i style="font-size:x-large" class="fa fa-trash text-danger"></i></button>
                                        </div>
                                    </td>
...
<!-- /.outer -->
<!-- Modal -->
<div class="modal fade" id="room_edit_modal" role="dialog" aria-labelledby="modalLabelprimary" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h4 class="modal-title text-white" id="modalLabelprimary">Add Room</h4>
            ...
            <form method="POST" action="{{ route('add_room') }}" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="col-lg input_field_sections">
                        <h4>Room Name</h4>
                        <input maxlength="50" type="text" class="form-control focused_input" name="room_name" id="room_name" required="required">
                    </div>
                </div>
...
<script>
    ...
    $(document).on('click', '.delete', function() {
        ...
        swal_confirm('Do you want to remove selected room?', function() {
            $.ajax({
                type: 'POST',
                url: "{{ url('/remove_room') }}",
                data: {
    ...
    $(document).on('click', '.edit', function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                $("#modalLabelprimary").html("Edit room");
                $("#room_name").val(obj['name']);
